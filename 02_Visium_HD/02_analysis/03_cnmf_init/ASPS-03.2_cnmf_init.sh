...
#SBATCH --signal=2
#SBATCH --no-requeue
#SBATCH --partition=cpu2023,cpu2022-bf24,cpu2022,cpu2021-bf24,cpu2021,cpu2019-bf05,cpu2019,cpu2017-bf05,single,sherlock,bigmem
...
RUN_NAME_VISHD="cNMF_5_100_5_F"
COUNT_FILE_VISHD="bS34RA_count_filtered_024um.tsv"
METADATA_FILE_VISHD="bS34RA_metadata.tsv"
OD_GENES="bS34RA_ODgenes_list_024um.txt"
...
mosaicmpi txt-to-h5ad --data ${COUNT_FILE_VISHD} --metadata ${METADATA_FILE_VISHD} -o dataset_vishd.h5ad
mosaicmpi check-h5ad -i dataset_vishd.h5ad -o dataset_vishd_filtered.h5ad
mosaicmpi select-hvf custom --name ${RUN_NAME_VISHD} --input dataset_vishd_filtered.h5ad --feature_list ${OD_GENES}
mosaicmpi initialize-cnmf --name ${RUN_NAME_VISHD} --k_range ${RANK_START} ${RANK_END} ${RANK_BY} --beta_loss ${BETA_LOSS}

# Submit the factorization script using its own sh script, so that we can utilize a Slurm array to speed it up.
sbatch cnmf_factorize.sh $PWD ${RUN_NAME_VISHD}

# Postprocess the factorization step.
mosaicmpi postprocess --output_dir $PWD --name ${RUN_NAME_VISHD} --cpus ${SLURM_JOB_CPUS_PER_NODE} --local_density_threshold 0.1
