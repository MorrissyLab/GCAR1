...
SAVE_FOLDER <- "02_Visium_HD/04_results/ASPS-03.2/"


PROGRAM_ANNOTATION <- NULL
RANK <- 10
GLOBAL_THRESHOLD <- 0.05
sample_metadata <- data.frame(sample = c("SPSQ_biopsy3_RA", "SPSQ_biopsy4_RA"))
sample_metadata$TREATMENT <- c("Biopsy_3", "Biopsy_4")
sample_metadata$LOCATION <- c(rep("Lung", 2))
sample_metadata$COLS <- paletteer_d("ggsci::dark_uchicago")[4:5]
...
SPECTRA <- "02_Visium_HD/03_datasets/02_cnmf_outputs/ASPS-03.2/cNMF_5_100_5_F.gene_spectra_score.k_10.dt_0_1.txt"
...
usage <- "02_Visium_HD/03_datasets/02_cnmf_outputs/ASPS-03.2/cNMF_5_100_5_F.usages.k_10.dt_0_1.consensus.txt"
...
program_annotation_list <- c("T_MES", "T", "T", "endo_capillary",
                             "endo_arteriole", "T", "macrophage",
                             "CAF_ECM", "lung_fibroblast", "CD8")
...
NEIGHBORHOOD_ANALYSIS_SUMMARY_PROGRAMS <- c("10")
...
program_enrichment <- function(usage_norm, threshold = 0.1, plot = "barplot", file_name, file_height = 6, file_width = 8, program_annotation = NULL){
  ...
  if (plot == "barplot"){
    ...
    temp_p2_df <- p2_df
    temp_p2_df$Sample <- factor(temp_p2_df$Sample, levels = c("Biopsy_3", "Biopsy_4"))
...
# so_list <- sort(ls()[grepl("SPSQ", ls())])

# 2. Reviewer loading
SPSQ_biopsy3_RA <- readRDS("02_Visium_HD/03_datasets/01_seurat_objects/ASPS-03.2/biopsy_sample_1_seurat_object.rds")
SPSQ_biopsy4_RA <- readRDS("02_Visium_HD/03_datasets/01_seurat_objects/ASPS-03.2/biopsy_sample_2_seurat_object.rds")
...
SPSQ_biopsy3_RA@meta.data["Sample"] <- "SPSQ_biopsy3_RA"
SPSQ_biopsy4_RA@meta.data["Sample"] <- "SPSQ_biopsy4_RA"
...
if (NEIGHBORHOOD_ANALYSIS_PROGRAMS || NEIGHBORHOOD_ANALYSIS_SUMMARY ||
  ...
  if (!file.exists(paste("02_Visium_HD/03_datasets/04_bin_neighbours/ASPS-03.2/", "spot_neighbours_df.rds", sep = ""))){
  ...
  } else{spot_neighbours_df <- readRDS(paste("02_Visium_HD/03_datasets/04_bin_neighbours/ASPS-03.2/", "spot_neighbours_df.rds", sep = ""))}
  print("Spot Neighbours Dataframe Created and Saved:")
  print(Sys.time())
  if (!file.exists(paste("02_Visium_HD/03_datasets/04_bin_neighbours/ASPS-03.2/", "spot_neighbours_df_wo_overlaps.rds", sep = ""))){
  ...
  } else{spot_neighbours_df_wo_overlaps <- readRDS(paste("02_Visium_HD/03_datasets/04_bin_neighbours/ASPS-03.2/", "spot_neighbours_df_wo_overlaps.rds", sep = ""))}
...
# 3.1 Neighbourhood of each program ----
...
if (NEIGHBORHOOD_ANALYSIS_SUMMARY){
  ...
  for (program_selection in NEIGHBORHOOD_ANALYSIS_SUMMARY_PROGRAMS){
    ...
    # Filter the niche
    filtering_list <- list("1,SPSQ_biopsy3_RA" = c(19),
                           "1,SPSQ_biopsy3_RA,1" = c(14,15),
                           "1,SPSQ_biopsy3_RA" = c(3),
                           "1,SPSQ_biopsy3_RA,2" = c(8),
                           "1,SPSQ_biopsy3_RA,2" = c(4),
                           "1,SPSQ_biopsy3_RA" = c(8))
    ...
    plot_height <- (0.1969*nrow(spot_neighbour_programs_df_flat_plot)) + 1.3386 + (length(haT@anno_size) * 5 / 10)
    pdf(paste0(save_path, "GEP5_T", usage_threshold, "_Binarized_T", Binarized_threshold, "_Dendro_", dendro_split, "_Split_Custom.pdf"), width = 20, height = plot_height)
    draw(ht_combined, padding = unit(c(3, 3, 3, 3), "cm"))
    dev.off()
...
if (INSIDE_OUTSIDE_NICHE){
  ...
  # 2.1 Load the data ----
  var_save_root <- paste0(SAVE_FOLDER, "K", RANK, "/neighbourhood_analysis/Programs/GEP_10/")
      ...
      if (gene %in% stats$gene){
        ...
        gp <- ggplot(temp_plot_data, aes(x = radius, y = proportion_gene_pos_bins, fill = Split_V2)) +
          ...
          theme(axis.text.x = element_text(angle = 45, hjust = 1, colour="black"),
                axis.text.y=element_text(colour="black"), text = element_text(size = 20),
                legend.position = "bottom") +
          labs(x = paste("Distance From GEP_10, Niche: ", niche, sep = ""),
               y = "Proportion of Gene+ Spots\nof Total Spots Per Condition",
               title = paste(gene, pair, sep = ": "),
               subtitle = paste("Fraction of Adjacent and Distant Bins GEP_10, Niche: ", niche, sep = "")) +
...
if (DEG_ALDEX_ANALYSIS){
  ...
  combined_layers_duplicate@meta.data$pre_post[combined_layers_duplicate@meta.data$sample == "SPSQ_biopsy3_RA"] <- "Pre_CAR_Dose"
  combined_layers_duplicate@meta.data$pre_post[combined_layers_duplicate@meta.data$sample == "SPSQ_biopsy4_RA"] <- "Post_CAR_Dose"
  ...
  conds <- ifelse(conds == "SPSQ_biopsy3_RA", "Pre_CAR_Dose", "Post_CAR_Dose")
  ...
  # 3.8 Plot the DEG between the pre- and post-CAR bins result ----
  pdf(paste0(save_path, "DEG_pre_post_car.pdf"))
  for (program in unique(pre_post_deg_list$column_label)){
    plot_df <- pre_post_deg_list[pre_post_deg_list$column_label == program,]

    cols <- colnames(plot_df)[grep("rab.win", colnames(plot_df))]

    if (program == "all_bins_individual"){
      print(plot_volcano(plot_df, logfc_threshold = 1, caps = F, top_n_labels = 50,
                         title = paste0(program, ": Pre vs Post CAR"), subtitle = paste0("Pre-CAR Bins: ", plot_df$Pre_CAR_Dose,
                                                                                         ", Post-Car Bins: ", plot_df$Post_CAR_Dose,
                                                                                         "\nLeft is: ", cols[1],
                                                                                         "\nRight is: ", cols[2])))
    } else if (program == "all_bins_pseudobulk"){
      print(plot_volcano(plot_df, logfc_threshold = 1, caps = F, top_n_labels = 50,
                         title = paste0(program, ": Pre vs Post CAR"), subtitle = paste0("Pre-CAR Pseudobulks: ", plot_df$Pre_CAR_Dose,
                                                                                         ", Post-Car Pseudobulks: ", plot_df$Post_CAR_Dose,
                                                                                         # "\n[Number of Pseudobulk Per Sample: 7]",
                                                                                         "\n[Number of Bins per Pseudobulk: 20]",
                                                                                         # "\n[Number of Counts per Pseudobulk: 1e6]",
                                                                                         "\nLeft is: ", cols[1],
                                                                                         "\nRight is: ", cols[2])))
    } else{
      print(plot_volcano(plot_df, logfc_threshold = 1, caps = F, top_n_labels = 50,
  ...
  # 3.9 Find the significant genes across all program DGE ----
  # 3.9.1 Check for gene significance ----
  ...
  colnames(scaled_mat) <- program_annotations
  label_colors <- brewer.pal(n = length(program_annotations), name = "Set3")
  ...
  pdf(paste(save_path, "Sig_gene_heatmap_pseudobulk.pdf", sep = ""), height = 60, width = 25)
  ...
  pdf(paste(save_path, "Sig_genes_gene_expression_program_usage_correlation.pdf", sep = ""), width = 40, height = 60)
  print(plot_grid(p1, p2))
  print(plot_grid(p3, p4))
  print(plot_grid(p5, p6))
  print(plot_grid(p7, p8))
...
# 4.4 Proportion of GCAR+ bins among T-cell program+ bins ----
if (BIN_FRACTION){
  # 4.0 Clear existing non-function variables, set the save folders (create them if they doesn't exist) ----
  rm(list = setdiff(setdiff(ls(), lsf.str()), program_var_names))
  ...
  # 4.2 Calculate GCAR+ proportion among program_1 positive bins ----
  gene <- "GCAR"
  program_1 <- 10
